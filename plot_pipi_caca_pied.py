...
# Color palette + marker styles per word (tweak freely)
STYLE = {
    "pipi": {"color": "#FF6B6B", "marker": "o"},
    "caca": {"color": "#4D96FF", "marker": "s"},
    "pied": {"color": "#51CF66", "marker": "^"},
}
...
def count_target_words(transcription):
    clean = re.sub(r"[^\w\s'-]", "", transcription.lower())
    words = clean.split()
    return {word: sum(1 for w in words if w == word) for word in target_words}


def build_series(data, min_upload_date="202603"):
    """Extract sorted timestamps + cumulative per-word counts."""
    timestamps = []
    counts_dict = defaultdict(list)

    for video_info, transcription in data:
        if video_info["upload_date"] < min_upload_date:
            continue
        timestamps.append(video_info["timestamp"])
        counts = count_target_words(transcription)
        for word in target_words:
    ...
    counts_sorted = {word: [counts_dict[word][i] for i in order] for word in target_words}
...
def plot_word_counts(data):
    ...
    for word in target_words:
        ...
        ax.fill_between(dates_sorted, counts_sorted[word], color=style["color"], alpha=0.06)

        # label the final value at the end of each line
        ax.annotate(
            f"{counts_sorted[word][-1]}",
            xy=(dates_sorted[-1], counts_sorted[word][-1]),
            xytext=(8, 0),
    ...
    # Titles & labels
    ax.set_title("Médolie : « pipi », « caca » ou « pied » ?", fontsize=18, fontweight="bold", pad=16)
